...
协议与 MCP 工具面;骨架不自动入库 `specs/`。契约 v3 已并入 grok 与 codex 对抗复审。

## 已定决策

- `scripts/olp-watch-board.sh` 新增 `--harvest <repo-root>`(放进既有 `case`,与 `--interval`/`--skip-signature`
  并存):命中 token 时先 `bash "$(dirname "$0")/olp-evo-harvest.sh" <repo-root>`,再打印 `BOARD-SIGNAL: <token>`
...
- `scripts/olp-evo-spec-skeleton.sh <FLAW-NNN.md> [--out <file>]`(bash + python3,import lib):输出中文段头契约
  ...
  `pending_item_<n>`)/ 假设·当·那么三行占位;缺段 `<!-- TODO: <段名> -->`;`satisfies: [<req>]` 或 `[]` + `## 问题`
  列 `- 未绑定需求`。缺省 stdout;`--out` 目标经 realpath 若在仓库内且不以 `specs/drafts/` 开头则退出 2 并 stderr
...
- `scripts/olp-evo-metrics.sh` 新增 `--stall <review-board>`、`--stall-threshold <minutes>`(缺省 0)、`--now <ISO8601>`
  ...
  `不派单`、`立案并派单`、`立案+派单` 因前一字符非空白/非行首或后无片号而不匹配;ACK 行匹配
  `^(?:> )?ACK\((?P<s>S)\s+(?:done|blocked|wontdo)\b`,同片号取最长;派单时间 = 行内 `\((\d{4}-\d\d-\d\d) (\d\d:\d\d)`
  或 `\[(\d{4}-\d\d-\d\d)T(\d\d:\d\d)`,缺则所属 `### N.` 标题的 `\((\d{4}-\d\d-\d\d)` 按 00:00,仍缺为未知;
...
- 测试:`tests/olp_watch_board.rs`(新增五场景,既有四断言不动)、`tests/olp_evo_skeleton.rs`、`tests/olp_evo_index.rs`、
...
### Forbidden
- 不改 `src/**`、`.octos/loop.md`、`AGENTS.md`、`.claude/skills/**`(skill 步骤改动由主审另片提交)。
...
场景: 骨架拒绝写入 specs 根
  ...
  当 运行 olp-evo-spec-skeleton.sh
  那么 退出码等于 2 且 stderr 含 refusing to write outside specs/drafts/
  并且 该文件不存在
